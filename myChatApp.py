...
# ==========================================
# 1. 魂の注入：松本地域ガイド（知的な案内人）
# ==========================================
SYSTEM_PROMPT = """
あなたは、長野県松本市の歴史、文化、そして日常の美しさに精通した「地域ガイド」です。
...
# ==========================================
# 2. API設定（google_search_retrieval）
# ==========================================
try:
...
if "end_time" in st.session_state:
    ...
    if seconds_left > 0:
        ...
        if seconds_left <= 60:
            countdown_placeholder.error(f"⚠️ **出発まで あと {mins:02d}:{secs:02d}** （準備を始めましょう）")
        else:
            countdown_placeholder.info(f"⌛ **出発まで あと {mins:02d}:{secs:02d}** です。")
    else:
        countdown_placeholder.warning("🚌 お時間です。お気をつけていってらっしゃいませ。")
    ...
    with st.chat_message("assistant"):
        with st.spinner("情報を確認しています..."):
            try:
                ...
                st.error(f"エラーが発生しました。申し訳ありません。: {e}")